...
static int Save(SaveMeshType &m, const char * filename, bool binary=false)
{
  PlyInfo pi;
  return Save(m,filename,binary,pi);
}

static int Save(SaveMeshType &m,  const char * filename, int savemask, bool binary = false, CallBackPos *cb=0 )
{
  PlyInfo pi;
  pi.mask=savemask;
